...
double RFLPBudget_MyModel_Cplex_LP_ALL(int instanceNum, double B, int bRatio)
{
	IloEnv env;
	try {
		///////////////// read data file ////////////////////////////////
		//loadData(instanceNum);
		 //build the model		 
		//  nbCities=5;
		IloNumVarArray  XjU(env, nbCities, 0.0, 1);
		IloNumVarArray  XjR(env, nbCities, 0.0, 1);
		IloArray<IloNumVarArray>  Zij(env);
		for (IloInt i = 0; i < nbCities; i++)
			Zij.add(IloNumVarArray(env, nbCities, 0.0, IloInfinity, ILOFLOAT));
		IloArray<IloArray<IloNumVarArray>> Yikj(env);
		for (IloInt i = 0; i < nbCities; i++)
		{
			IloArray<IloNumVarArray> item(env);
			for (IloInt k = 0; k < nbCities; k++)
				item.add(IloNumVarArray(env, nbCities, 0.0, IloInfinity, ILOFLOAT));
			Yikj.add(item);
		}
		IloModel model(env);
	...
	expr_reliableNum.end();*/
		...
		outputFilename += "-RFLPwithBudget_Cplex_LP_ALL";
		stringstream ss;
		ss << nbCities;
		outputFilename += "-";
